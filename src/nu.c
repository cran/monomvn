...
double draw_nu_mh(const double nu_old, const unsigned int n, const double eta) 
{
  ...
  myprintf(stdout, "nu_old=%g, nu=%g, alpha=%g, qratio=%g", nu_old, nu, alpha, qb/qf);
  u = unif_rand();
  myprintf(stdout, " u=%g", u);
  if(u < alpha*qb/qf) {myprintf(stdout, " accept\n"); return nu; }
  else { myprintf(stdout, " reject\n"); return nu_old; }
}
